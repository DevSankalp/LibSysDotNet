...
namespace Library_Management_System.DATABASE
{
    ...
    {
        private Microsoft.Data.SqlClient.SqlConnection connection = null!; // initialized in Initialize()

        // connection string for Windows authentication and specific options
        // NOTE: include the database name using Initial Catalog (or Database) so the
        //       connection targets the correct database containing your tables.
        //       Replace "YourDatabaseName" with the actual name (e.g. LibraryManagement).
        private string connectionString =
            "Data Source=LP-TZD-0X140653;Initial Catalog=lms_db;Integrated Security=True;Persist Security Info=False;Pooling=False;MultipleActiveResultSets=False;Encrypt=True;TrustServerCertificate=True;Application Name=\"SQL Server Management Studio\";Command Timeout=0";
        ...
        {
            // using supplied connection string directly
        ...
        private bool OpenConnection()
        {
            ...
            {
                // handle common SQL Server error numbers
                switch (ex.Number)
                {
                    case 53: // network path not found / cannot connect
        ...
        {
            string query =
                "INSERT INTO Book(book_title, book_author, book_isbn, book_quantity, book_price) VALUES ('"
                + book.Title
                + "', '"
                + book.Author
                + "', '"
        ...
        {
            string query =
                "INSERT INTO Student(student_name, student_id, student_contact, student_email) VALUES ('"
                + student.Name
                + "', '"
                + student.ID
                + "', '"
        ...
        public void LoadAllBooks(DataGridView dataGridView)
        {
            ...
            if (this.OpenConnection())
            {
                try
                {
                    ...
                    Microsoft.Data.SqlClient.SqlDataAdapter dataAdapter =
                        new Microsoft.Data.SqlClient.SqlDataAdapter(cmd);
        ...
        public DataTable SearchAndLoadData(string searchText)
        {
            ...
            string query =
                "SELECT * FROM book WHERE book_title LIKE @SearchText OR book_author LIKE @SearchText OR book_isbn LIKE @SearchText";
        ...
        public DataTable SearchStudentData(string searchText)
        {
            ...
            string query =
                "SELECT * FROM student WHERE student_name LIKE @SearchText OR student_id LIKE @SearchText OR student_contact LIKE @SearchText";
        ...
        {
            string query =
                "UPDATE book SET book_quantity = book_quantity - @QuantityToSubtract WHERE id = @BookId";
        ...
        {
            string query =
                "UPDATE book SET book_quantity = book_quantity + @QuantityToReturn WHERE id = @BookId";
        ...
        {
            string query =
                "INSERT INTO bookreturn(book_id, quantity_subtracted, book_title, book_author, book_isbn, book_quantity, student_no) "
